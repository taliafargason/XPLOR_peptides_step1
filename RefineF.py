...
for (medium,expt,file,                 scale) in \
    [('t','NH' ,'RDC_unphos_sup_conf_struct.tbl'       ,1)]:
...
#pre = create_PREPot("PRE","220CPRE_Data.tbl","normal")

pre = PotList('pre')
a = create_PREPot("220C","220CPRE_Data.tbl","normal",fixTau=True)
a.setFunType("correlation")
#a.setTaucAtoms()
pre.append(a)
#b = create_PREPot("248C","248CPREdata.tbl","normal",fixTau=True)
...
for (name,scale,file) in [('all',1,"CSP_high_confidence_simplified.tbl"),
...
dyn.group("resid 16:90 and name C CA C N")
dyn.fix(f"({largeDomain}) and name C CA C N")
...
minc.group("resid 16:90 and name C CA C N")
minc.fix(f"({largeDomain}) and name C CA C N")
...
StructureLoop(numStructures=numberOfStructures,
              ...
              averageFitSel=f"({largeDomain}) and name CA",
